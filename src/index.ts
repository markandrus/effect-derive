...
export function main () {
  ...
  if (derivations.length === 0) {
    throw new Error(`At least one positional argument is required ("Covariant", "BaseFunctor", "Foldable" or "Traversable")`)
  }
  ...
  for (const derivation of derivations) {
    switch (derivation) {
      case 'Covariant':
        outFile.merge(deriveCovariant(inFilePath, forType, discriminator, registries.covariant, tyAliasDecl))
        break
      case 'BaseFunctor':
        outFile.merge(deriveBaseFunctor(inFilePath, forType, discriminator, registries.covariant, tyAliasDecl))
        break
      case 'Foldable':
        outFile.merge(deriveFoldable(inFilePath, forType, discriminator, registries.foldable, tyAliasDecl))
        break
      case 'Traversable':
        outFile.merge(deriveTraversable(inFilePath, forType, discriminator, registries.traversable, tyAliasDecl))
        break
      default:
